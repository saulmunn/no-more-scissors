...
**Safari on iPhone (and Mac):** Safari extensions ship inside an app. The `safari/` folder holds an Xcode project generated by Apple's converter. Open `safari/No More Scissors/No More Scissors.xcodeproj` in Xcode, pick your team under Signing, choose your iPhone, and Run. Then on the phone: Settings → Apps → Safari → Extensions → No More Scissors → on, and allow it on x.com when Safari asks. The project reads the extension files from `safari/extension/`, which is generated: run `scripts/sync-safari.sh` before the first build and after every change to the extension. A free Apple developer account works for your own devices (the install expires after a week); the App Store or TestFlight needs the paid program.
