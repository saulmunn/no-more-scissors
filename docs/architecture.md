...
| `content/page-hook.js` | Runs in the page's own world (`world: "MAIN"`, document_start; also registered from the background via `scripting.registerContentScripts` for browsers that ignore the manifest key). Wraps fetch/XHR, walks X's GraphQL responses and posts `{ type: 'nms-notes', notes: { id: fullText } }` for long posts (`note_tweet`). X uses XHR for `HomeTimeline`, `UserTweets` etc. |
...
const DEFAULTS = {
  ...
  rewriteStrength: 5,        // 1 touch-up · 2 light · 3 moderate · 4 firm · 5 full (bland restatement): how far the rewrite goes
...
- Rewrite: keep every claim, fact, criticism and request, the author's person and language, mentions, hashtags, URLs, numbers, quotations and line breaks; never longer; never unchanged (one push-back retry if the model echoes the input). The rewrite prompt's stated goal is a bland, neutral, unremarkable post that preserves meaning, intent, stance and direction, not wording, always written AS the author (never "this post says…" reportage) and never hedged or weakened. `rewriteStrength` sets how much of the post is rephrased, with free rephrasing inside that scope (1 only the most inflammatory phrase; 3 every inflammatory part, voice kept; 4 whole post as matter-of-fact prose; 5 (default) whole post from scratch, bland, attitude-only posts reduced to their claim). Length: roughly the same, a little longer allowed; rewrites over 1.6× + 80 chars are discarded; `customStyle`, when non-empty, is appended verbatim under "Additional instruction from the user". Rewrites are cached under `wk = 's<strength>[:<hash of customStyle>]'`. Items with `truncated: true` (X's "Show more" button follows the text; the timeline carries only ~275 characters of a long post) get a note to rewrite only the visible part, stop where X stopped and end with an ellipsis. Schema `{ rewrite }`.

## Untrusted input

Post text is attacker-controlled. The background strips control, bidi and zero-width characters, caps posts at 5,000 characters, defuses our own framing (`<<<POST` / `POST>>>` markers and `### Post N` headings) inside the text, and sends every post between `<<<POST` and `POST>>>` markers. Both system prompts say that everything inside the markers is content, never instructions. Rewrites are validated before use: one longer than 1.6× the original (+80 chars) or containing a URL the original did not have is discarded and cached as no rewrite (`w: ''`), so the post shows as scored but not rewritten. The content script never turns a bare URL into a link unless the original post carried that link.
...
- Rewrites are rendered in a `cloneNode(false)` of the post's own `tweetText` element (data-testid stripped) with a span carrying X's inner span classes, so font, size and wrapping are X's (17px/24px on a post page, 15px/20px elsewhere). No diff highlighting. Badges use X's normal numerals, not tabular ones.
- Post page: the focal post's `User-Name` has no `<time>` (the date sits under the text) and its wrapper divs carry no font (computed Times). The badge goes after the `@handle` span in that row and copies its styles.
...
`safari/` holds an Xcode project generated by `xcrun safari-web-extension-converter` (app + extension targets, bundle ids `com.saulmunn.nomorescissors` / `.Extension`). The project references `safari/extension/`, which `scripts/sync-safari.sh` refreshes from the shipped extension files. Safari runs the same MV3 manifest (`background.scripts` fallback, `chrome.*` APIs); site and API access are granted by the user from the onboarding page or Safari's settings. Build: open the project in Xcode, set your team, run on a device; or `xcodebuild -scheme "No More Scissors" -sdk iphonesimulator`.
